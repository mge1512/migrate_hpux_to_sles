...
if \[ \-f /etc/rc.config.d/custom\_app \] ; then  
   . /etc/rc.config.d/custom\_app  
fi
...
'start')  
    if; then  
        ...
        echo $\! \> /var/run/custom\_app.pid  
    fi  
    ;;  
'stop')  
    \# Fragile kill logic  
    if \[ \-f /var/run/custom\_app.pid \]; then  
        kill $(cat /var/run/custom\_app.pid)  
    fi  
    ;;  
esac
```
